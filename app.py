...
DB_PATH = "textile_inventory.db"

# ═══════════════════════════════════════════════
# DATABASE SYSTEM
# ═══════════════════════════════════════════════
# PERFORMANCE FIX: table creation / column-migration / index creation used to
# run on EVERY single get_conn() call (i.e. dozens of times per page rerun —
# once per q()/scalar() call). As the data grew this became the main source of
# slowness across searching, inline edit, and saving. It now runs exactly ONCE
# per app session via st.cache_resource, and get_conn() only opens a plain
# connection with fast PRAGMAs afterwards.
...
def _init_schema():
    ...
    # NEW: Bilty (Lahore factory -> Karachi dispatch) ledger. This is kept as
    # its own separate table so the existing Master Ledger (Ordered vs
    # Received, driven by `inventory`) is never touched by Bilty saves.
    conn.execute("""
    ...
    for col in ["entry_date","remark","company_token","contract_no","style_type"]:
        if col not in inv_cols:
            conn.execute(f"ALTER TABLE inventory ADD COLUMN {col} TEXT DEFAULT ''")

    # WAL mode = much faster + more concurrent-friendly reads/writes than the
    # default rollback-journal mode, especially once the .db file grows large.
    conn.execute("PRAGMA journal_mode=WAL")
    conn.execute("PRAGMA synchronous=NORMAL")

    # Indexes on every column used in WHERE / JOIN / GROUP BY across the app
    # (Global Search, DC Entry auto-load, All Entries filters, Master Ledger
    # join) so lookups stay fast as rows increase instead of full-table scans.
    conn.execute("CREATE INDEX IF NOT EXISTS idx_inv_article_category ON inventory(article, category)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_inv_calloff           ON inventory(call_off_no)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_inv_contract          ON inventory(contract_no)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_inv_po                ON inventory(po_no)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_inv_dc                ON inventory(dc_no)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_inv_token             ON inventory(company_token)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_inv_entrydate         ON inventory(entry_date)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_so_calloff            ON sheet_orders(call_off_no)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_so_article_category   ON sheet_orders(article, category)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_so_contract           ON sheet_orders(sale_contract)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_so_po                 ON sheet_orders(po_no)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_bilty_calloff_art     ON bilty(call_off_no, article, category)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_bilty_contract        ON bilty(contract_no)")
...
tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs([
    ...
    "📊 Master Ledger","📤 Sheet Upload","🚚 Bilty Management"
])
...
<div class="footer">
  ...
  Customer: Vertex (Shahzad Bhai) Lahore &nbsp;|&nbsp; v6.4
</div>
""", unsafe_allow_html=True)
