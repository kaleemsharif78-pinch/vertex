...
with tab6:
    if _access_ok("🚚 Bilty Management"):
        ...
        b_c1, b_c2, b_c3 = st.columns([2, 2, 1.3])
        ...
        with b_c3:
            pack_qty = st.number_input("Fixed Pack Qty / Tick", min_value=1.0, value=25.0, step=1.0, key="bilty_pack_qty")

        if b_sel_coff == "-- Select --" or b_sel_cont == "-- Select --":
            st.info("👆 Please select a Call-Off Sheet and Sales Contract to load its articles.")
        else:
            df_arts = q("""
                SELECT DISTINCT article, category FROM sheet_orders
                ...
                for art, grp in df_arts.groupby("article"):
                    with st.expander(f"📦 Article: {art}", expanded=True):
                        ...
                        for i, (_, r_row) in enumerate(grp.iterrows()):
                            ...
                            with tcols[i % 3]:
                                is_checked = st.checkbox(f"{cat} ({pack_qty:,.0f} Pcs)", key=cb_key)
                            if is_checked:
                                grand_total += pack_qty
                                ticked_items.append((art, cat))
